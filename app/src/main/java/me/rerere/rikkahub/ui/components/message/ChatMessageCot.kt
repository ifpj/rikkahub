...
/**
 * 思考步骤类型，用于分组 Reasoning、Tool 和 WebSearch
 */
sealed interface ThinkingStep {
    ...
    data class WebSearchStep(
        val search: UIMessagePart.WebSearch,
    ) : ThinkingStep
}
 ...
 * 连续的 Reasoning、Tool 和 WebSearch 会被分组到一个 ThinkingBlock 中
 */
    ...
    this.fastForEachIndexed { index, part ->
        when (part) {
            ...
            is UIMessagePart.WebSearch -> {
                currentThinkingSteps.add(ThinkingStep.WebSearchStep(part))
            }
